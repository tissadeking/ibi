...
def get_stored_intent():
    ...
    if intent_index == True:
        resp = es.search(index='stored_intents', size=200, query={"match_all": {}})
        #source_arr = []
    ...
    if len(source_arr) > 0:
        ...
        for i in range(len(source_arr)):
            got_intent = Stored_intent(id=i+1, intent_type=source_arr[i]['intent_type'],
            threat=source_arr[i]['threat'], host=source_arr[i]['host'], action=source_arr[i]['action'],
            duration=source_arr[i]['duration'], intent_id=source_arr[i]['intent_id'],
            priority=source_arr[i]['priority'], actual_time=source_arr[i]['actual_time'])
            stored_intents.append(got_intent)
    else:
        stored_intents.clear()
        got_intent = Stored_intent(id=0, intent_type='', threat='', host='', action='',
                      duration='', intent_id='', priority='', actual_time=0.0)

        stored_intents.append(got_intent)

    #print('get request stored intents', stored_intents)
...
def get_stored_qos_intent():
    ...
    if intent_index == True:
        resp = es.search(index='stored_qos_intents', size=200, query={"match_all": {}})
        # source_arr = []
        for hit in resp['hits']['hits']:
            source_arr.append(hit["_source"])
    if len(source_arr) > 0:
        ...
        for i in range(len(source_arr)):
            got_qos_intent = Stored_qos_intent(id=i + 1, intent_type=source_arr[i]['intent_type'],
            name=source_arr[i]['name'], value=source_arr[i]['value'], unit=source_arr[i]['unit'],
            host=source_arr[i]['host'], qos_intent_id=source_arr[i]['qos_intent_id'])
            stored_qos_intents.append(got_qos_intent)
    else:
        stored_qos_intents.clear()
        got_qos_intent = Stored_qos_intent(id=0, intent_type='', name='', value=0.0,
                                    unit='', host='', qos_intent_id='')
        stored_qos_intents.append(got_qos_intent)
        #stored_qos_intents = [Stored_qos_intent(id=0, intent_type='', name='', value=0.0,
        #                                        unit='', host='', qos_intent_id='')]
    # print('get request stored intents', stored_qos_intents)
